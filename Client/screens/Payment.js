...
const Payment = (props) => {
  ...
  const sendData = async (clientSecret) => {
    const values = { "Date": data.date, "StartTime": data.startTime, "EndTime": data.endTime, "VehicleNo": data.vehicleNo, "slot": data.slot, "user_email": data.userName, "currentTime": moment.tz("Asia/Colombo").format("HH:mm:ss"), "paymentAmount": data.Total, "paymentIntent": clientSecret, "timeDiff": data.timeDiff }
    const response = await axios.post(server + 'successBooking', values);
    if (response.data == 200) {
      props.navigation.reset({
        index: 0,
        routes: [{
          name: 'RefundSuccess',
          params: { Message: "Payment is succefull and booking has been recorded." }
        }]
      })
    } else {
      setErrTitle("Oops...!!");
